...
export default function Button({ href, children, onClick, className, type='button', ariaLabel }: Props) {
  ...
  if (href) {
    return <Link href={href} aria-label={ariaLabel} className={clsx(base, className)}>{children}</Link>
  }
  return <button type={type} onClick={onClick} aria-label={ariaLabel} className={clsx(base, className)}>{children}</button>
}
