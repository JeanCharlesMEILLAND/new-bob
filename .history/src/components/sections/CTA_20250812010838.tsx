    ...
    <section id="waitlist" className="section">
      <Container className="text-center">
        ...
        <p className="mt-3 text-white/80">
          Soyez les premiers informés du lancement, recevez des accès anticipés et des bonus de démarrage.
        </p>
        <form onSubmit={join} className="mx-auto mt-8 flex max-w-md items-center gap-2">
          ...
          <Button type="submit" variant="primary" size="md" className="whitespace-nowrap">
            {status === 'loading' ? 'Envoi…' : status === 'ok' ? 'Inscrit ✔︎' : 'S’inscrire'}
          </Button>
        </form>
        {status === 'err' && <p className="mt-2 text-red-300">Oups, une erreur est survenue. Réessayez.</p>}
      </Container>
    </section>
  )
}
